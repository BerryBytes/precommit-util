...
# ----------------------------------------
# Sync file to precommitFiles folder
# ----------------------------------------
sync_to_precommitFiles() {
    local source_file=".pre-commit-config.yaml"
    local target_dir="precommitFiles"
    local target_file="$target_dir/.pre-commit-config.yaml"

    mkdir -p "$target_dir"

    if [[ ! -f "$target_file" ]]; then
        log "STEP" "Copying $source_file → $target_file"
        cp "$source_file" "$target_file"
        log "INFO" "Created $target_file"
    else
        # Only update if content is different
        if ! cmp -s "$source_file" "$target_file"; then
            log "STEP" "Updating $target_file (syncing changes)"
            cp "$source_file" "$target_file"
            log "INFO" "Updated $target_file with latest changes"
        else
            log "INFO" "No changes detected — precommitFiles already synced"
        fi
    fi
}
