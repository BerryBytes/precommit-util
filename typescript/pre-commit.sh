...
check_dependencies() {
    local required_tools=("pre-commit")
    local missing_deps=()
...
run_formatting_hooks() {
    ...
    local formatting_hooks=("check-yaml" "end-of-file-fixer" "trailing-whitespace" "check-added-large-files" "prettier" "stylelint" "codespell" "conventional-pre-commit" )
    local exit_code=0
